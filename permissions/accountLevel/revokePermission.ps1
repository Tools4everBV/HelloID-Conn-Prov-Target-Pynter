###############################################################
# HelloID-Conn-Prov-Target-Pynter-RevokePermission-AccountLevel
# PowerShell V2
###############################################################
...
try {
    ...
    switch ($action) {
        'RevokePermission' {
            ...
            if ($personUpdateObject.PSObject.Properties.Name -Contains 'AccountLevel') {
                $personUpdateObject.PSObject.Properties.Remove('AccountLevel')
            }
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Revoking Pynter permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"
                ...
                Write-Information "[DryRun] Revoke Pynter permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not revoke Pynter permission. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditMessage = "Could not revoke Pynter permission. Error: $($_.Exception.Message)"
