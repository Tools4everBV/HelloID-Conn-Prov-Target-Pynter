#################################################
# HelloID-Conn-Prov-Target-Pynter-Enable
# PowerShell V2
#################################################
...
try {
    ...
    switch ($action) {
        'EnableAccount' {
            ...
            $splatUpdatePersonXmlBody = @{
                ...
                Parameters = @{ pynterPersonId = $actionContext.References.Account; personUpdate = $accountEnableObject }
            }
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                ...
                Write-Information "[DryRun] Enable Pynter account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not enable Pynter account. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not enable Pynter account. Error: $($_.Exception.Message)"
